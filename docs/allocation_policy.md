...
8. **MANAGER_CENTER_GATE**: اگر منتور مدیر دارد، مرکز دانش‌آموز باید در فهرست مراکز مجاز آن مدیر باشد؛ در غیر این صورت رد با دلیل `manager_centers_not_found` ثبت می‌شود. پیاده‌سازی پایدار از جدول‌های `managers` و `manager_allowed_centers` با کش ثانیه‌ای (`DatabaseManagerCentersProvider`) داده‌ها را واکشی می‌کند تا سربار پرس‌وجو کاهش یابد.
