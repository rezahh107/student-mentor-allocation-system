...
- ترتیب اعمال لایه‌ها به صورت `Observability → CorrelationID → ContentType → BodySize → Authentication → Idempotency → RateLimit → Router` است؛
  این چینش باعث می‌شود Observability از ابتدا شناسهٔ همبستگی را ثبت کند، نگهبان‌های محتوا/حجم بدنه خطای ۴۲۲ با بدنهٔ JSON استاندارد تولید کنند،
  و ایدمپوتنسی پیش از مصرف توکن‌های نرخ قرار گیرد تا بازپخش‌ها بدون کاهش سهمیه پاسخ دهند.
