...
class RedisRateLimitBackend:
    ...
    async def consume(
        ...
        *,
        ...
        allowed, remaining, retry_after = (float(result[0]), float(result[1]), float(result[2]))
        return RateLimitDecision(allowed=bool(int(allowed)), remaining=remaining, retry_after=retry_after)
