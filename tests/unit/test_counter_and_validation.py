...
def test_counter_format():
    backend = MockBackend()
    c_m = asyncio.run(backend.get_next_counter(1))
    c_f = asyncio.run(backend.get_next_counter(0))
    assert len(c_m) == 9 and len(c_f) == 9
    assert c_m[2:5] == "373"
    assert c_f[2:5] == "357"


def test_counter_concurrency_uniqueness():
    backend = MockBackend()
    async def get_one():
        g = random.choice([0, 1])
    ...
    async def run_all() -> list[str]:
        return await asyncio.gather(*[get_one() for _ in range(200)])

    vals = asyncio.run(run_all())
