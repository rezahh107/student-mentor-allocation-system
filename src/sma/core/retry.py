...
class RetryMetrics:
    ...
    def record_attempt(self, *, op: str, outcome: str, backoff: float | None) -> None:
        self.retry_attempts_total.labels(op=op, outcome=outcome).inc()
        if backoff is not None:
            self.retry_backoff_seconds.labels(op=op).observe(backoff)

    def record_exhaustion(self, *, op: str) -> None:
        self.retry_exhausted_total.labels(op=op).inc()


def _build_default_metrics() -> RetryMetrics:
    attempts = _safe_metric(
        Counter,
        "retry_attempts_total",
        "Total retry attempts per operation and outcome.",
        labelnames=("op", "outcome"),
    )
    exhausted = _safe_metric(
        Counter,
        "retry_exhausted_total",
        "Total number of times retries were exhausted.",
        labelnames=("op",),
    )
    backoff_hist = _safe_metric(
        Histogram,
    ...
    return RetryMetrics(
        retry_attempts_total=attempts,
        retry_exhausted_total=exhausted,
        retry_backoff_seconds=backoff_hist,
    )


_DEFAULT_RETRY_METRICS = _build_default_metrics()

retry_attempts_total = _DEFAULT_RETRY_METRICS.retry_attempts_total
retry_exhausted_total = _DEFAULT_RETRY_METRICS.retry_exhausted_total
retry_backoff_seconds = _DEFAULT_RETRY_METRICS.retry_backoff_seconds
# Backwards-compatible alias retained for legacy integrations.
retry_exhaustion_total = retry_exhausted_total
...
def _record_metrics(
    *,
    ...
    outcome: str,
    backoff: float | None,
    metrics: RetryMetrics | None,
) -> None:
    _DEFAULT_RETRY_METRICS.record_attempt(op=op, outcome=outcome, backoff=backoff)
    if metrics is not None and metrics is not _DEFAULT_RETRY_METRICS:
        metrics.record_attempt(op=op, outcome=outcome, backoff=backoff)
    ...
    for attempt in range(1, policy.max_attempts + 1):
        try:
            ...
            if not _should_retry(exc, retryable) or attempt >= policy.max_attempts:
                ...
                _DEFAULT_RETRY_METRICS.record_exhaustion(op=op)
                if metrics is not None and metrics is not _DEFAULT_RETRY_METRICS:
                    metrics.record_exhaustion(op=op)
...
def build_retry_metrics(
    *, namespace: str, registry: CollectorRegistry | None = None
) -> RetryMetrics:
