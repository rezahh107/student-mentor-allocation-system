...
@pytest.fixture()
def secured_app(httpx_client_factory) -> Any:
...
def test_metrics_requires_token(secured_app: Any) -> None:
    response = secured_app.get("/metrics")
    assert response.status_code == 401
    assert response.json()["error"]["code"] == "AUTH_REQUIRED"


def test_metrics_wrong_ip_forbidden(secured_app: Any) -> None:
...
def test_jwt_issuer_and_audience_validation(secured_app: Any) -> None:
    now = int(time.time())
