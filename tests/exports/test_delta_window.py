...
from sma.phase6_import_to_sabt.models import ExportDeltaWindow, ExportFilters, ExportOptions, ExportSnapshot
...
def test_delta_no_gap_overlap(tmp_path) -> None:
    ...
    snapshot = ExportSnapshot(marker="marker", created_at=base)
    ...
    assert manifest.delta_window == delta
    assert manifest.metadata["files_order"]
