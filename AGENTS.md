...
* **Observability:** JSON logs with **Correlation-ID**.
...
4. Validate: `/openapi.json`, `/docs`, `/redoc` (when PUBLIC_DOCS=true); `/metrics` endpoint available.
...
## 9) User-Visible Errors (Persian, deterministic)
...
## 10) Operational Mode — Debug-First (80/20)

1. **Analyze & profile:** Apply the **5-Layer** framework (§12) + cProfile + tracemalloc; **quantify** issues (e.g., p95=22s; +50MB).
...
## 11) Agent Playbook (debug/perf tasks)
...
## 12) Debugging & RCA Framework (5 layers)
...
## 13) Definition of Done (performance-first)
...
## 14) Quick Reference & Decision Tree (perf-centric)
...
* **Tests:** `clean_state`, `injected_clock` fixtures required | Warnings = 0 | Evidence per §1—§14.
