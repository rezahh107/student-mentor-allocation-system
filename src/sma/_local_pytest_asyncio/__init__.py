...
_REAL = load_real_plugin("pytest_asyncio", current_module=_CURRENT)
...
if _REAL is not None:
    ...
    exported = {
        key: value
        for key, value in _REAL.__dict__.items()
        if key != "__name__"
    }
    _CURRENT.__dict__.update(exported)
    STUB_ACTIVE: Final[bool] = False
else:
    STUB_ACTIVE: Final[bool] = True

    # pragma: no cover - compatibility no-op
