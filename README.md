...
- اجرای آزمون‌های سخت‌سازی اکنون بدون پرچم‌های اضافی انجام می‌شود: `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -p pytest_asyncio.plugin tests/hardened_api -q`. حالت پیش‌فرض asyncio در `pytest.ini` روی `asyncio_mode=auto` ثابت شده و دیگر نیاز به `-o asyncio_default_fixture_loop_scope=function` نیست.
