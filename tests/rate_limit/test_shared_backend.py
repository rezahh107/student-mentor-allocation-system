...
except ImportError:  # pragma: no cover - missing fastapi
    pytest.xfail(
        "FastAPI test client is unavailable. Install fastapi>=0.110 and starlette>=0.27 to run hardened Redis suites.",
    )
...
@pytest.fixture()
def clean_state(redis_client: "redis.Redis") -> Iterator[None]:
...
def _debug_context(redis_client: "redis.Redis", namespace: str) -> str:
    keys = sorted(
...
def _distributed_worker(args: tuple[str, str, str, dict[str, object], str]) -> tuple[int, dict[str, str]]:
    ...
    client = _build_app(config, allocator)
...
def test_rate_limit_shared_across_instances(
    redis_url: str, redis_client: "redis.Redis", _clean_state: None
) -> None:
...
def test_middleware_order_rate_limit_before_idempotency(
    redis_url: str, _clean_state: None
) -> None:
...
def test_dirty_state_is_cleaned(redis_url: str, redis_client: "redis.Redis", _clean_state: None) -> None:
...
def test_idempotency_backend_degraded_mode(redis_url: str, redis_client: "redis.Redis", _clean_state: None) -> None:
